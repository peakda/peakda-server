...
class MyPageServiceTest {
    ...
    @Test
    fun `사용자가 없으면 UserNotFoundException 이다`() {
        `when`(userRepository.findById(USER_ID)).thenReturn(Optional.empty())

        assertThatThrownBy { service.getMyPage(USER_ID) }.isInstanceOf(UserNotFoundException::class.java)
    }
    ...
    fun `통계·관심꽃·기록그리드를 조합한다`() {
        ...
        `when`(profileImageUrlResolver.mainUrl("profile-images/1/main.jpg")).thenReturn("https://cdn/main.jpg")
        ...
        assertThat(response.stats.favoriteSpotCount).isEqualTo(8)
    ...
    private fun record(id: Long): SpotRecord {
        val record = SpotRecord(spotId = 100L, userId = USER_ID, status = SpotRecordStatus.PUBLISHED)
    ...
    private fun summary(id: Long) = SpotRecordSummaryResponse(
        ...
        spotId = 100L,
        spotName = "남산",
        user = SpotRecordResponse.UserSummary(id = USER_ID, nickname = "벚꽃러버", profileImageUrl = null),
        visitedDate = null,
        bloomStage = null,
        memo = null,
        plants = emptyList(),
