...
class KorServiceSyncJobTest {
    ...
    private val materializationService = NoOpMaterializationService()
    ...
    fun `run 시 areaBasedSyncList2를 호출해 sync service에 페이지를 전달한다`() {
        ...
        val job = KorServiceSyncJob(fixture.client, syncService, materializationService, enabled(true), testJobLogger())
        job.run()
    ...
    fun `enabled=false 이면 client와 sync service 모두 호출하지 않는다`() {
        val job = KorServiceSyncJob(fixture.client, syncService, materializationService, enabled(false), testJobLogger())
    ...
    private class NoOpMaterializationService :
